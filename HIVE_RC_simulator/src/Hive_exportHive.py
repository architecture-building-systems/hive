...
"""
Based on Ladybug_Export Ladybug. Code Developers of Hive can use this component to export user objects and source code that they create to the Github folder on their computer.
...
Provided by Hive 0.0.1
    ...
    Returns:
        readMe!: ...
"""
ghenv.Component.Name = "Hive_exportHive"
...
def exportToUserObject(component, targetFolder, hive_preparation):
    ...
    def isNewerVersion(currentUO, component):
        ...
        # print version, date
        month = hive_preparation.monthList.index(month.upper()) + 1
        ghCompDate = int(hive_preparation.getJD(month, day))
        ...
        for lineCount, line in enumerate(componentCode):
            ...
            if line.strip().startswith("ghenv.Component.Message"):
                ...
                month = hive_preparation.monthList.index(month.upper()) + 1
                UODate = int(hive_preparation.getJD(month, day))
                break
    ...
    if component.Name.find("Hive")>=0:
        ...
        if currentUO!=None:
            print isNewerVersion(currentUO,component)
            # if is newer remove
        ...
        try:
            uo.Exposure = exposureDict[int(component.Code.split('ComponentExposure=')[1].split('\n')[0])]
        except:
        ...
        if component.Category == "Maths":
            uo.Description.Category = "Hive"
        else:
            uo.Description.Category = component.Category
            
        if component.SubCategory == "Script":
            uo.Description.SubCategory = "UnknownHiveComponents"
        else:
...
def exportToFile(component, targetFolder, hive_preparation):
        ...
        with open(os.path.join(targetFolder,fileName), "r") as pyFile:
            for lineCount, line in enumerate(pyFile):
                ...
                if line.strip().startswith("ghenv.Component.Message"):
                    ...
                    month = hive_preparation.monthList.index(month.upper()) + 1
                    pyFileDate = int(hive_preparation.getJD(month, day))
                    break
...
def main(components, targetFolder):
    if not sc.sticky.has_key('Element'): return "Add the modular building physics component to the canvas!"
    ...
    for ghComp in ghComps:
        fileExported = exportToFile(ghComp, targetFolder, hive_preparation)
        if fileExported:
            exportToUserObject(ghComp, targetFolder, hive_preparation)
