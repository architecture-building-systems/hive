...
namespace Hive.GUI
{
    public class ClusterTest : GH_Component, IGH_VariableParameterComponent, IGH_InstanceGuidDependent, IGH_DocumentOwner
    {
        ...
        private ClusterTest.HookParamMap m_mapping;
                 ...
                 "GUI")
        {
            ...
            this.m_mapping = new ClusterTest.HookParamMap();
            ...
            switch (hook.Recipients.Count)
            {
                ...
                case 1:
                    ...
                    if (ghParam != null)
                    {
                        ...
                        goto case 0;
                default:
                    Guid id = ClusterTest.SharedTypeIDs((IEnumerable<IGH_Param>)hook.Recipients);
                    if (id != Guid.Empty)
                    {
            ...
            switch (hook.Sources.Count)
            {
                ...
                case 1:
                    ...
                    if (ghParam != null)
                    {
                        ...
                        goto case 0;
                default:
                    Guid id = ClusterTest.SharedTypeIDs((IEnumerable<IGH_Param>)hook.Sources);
                    if (id != Guid.Empty)
                    {
                        IGH_DocumentObject ghDocumentObject = Instances.ComponentServer.EmitObject(id);
                        if (ghDocumentObject != null && ghDocumentObject is IGH_Param)
        ...
        {
            ClusterTest.HookParamMap hookParamMap = new ClusterTest.HookParamMap();
            IEnumerator<KeyValuePair<Guid, Guid>> enumerator;
            try
            {
                enumerator = this.m_mapping.ParamToHook.GetEnumerator();
        ...
        public void DocumentModified(GH_Document modifiedDocument)
        {
            ...
            IEnumerator<ClusterTest> enumerator1;
            try
            {
                enumerator1 = this.ClusterFamily().GetEnumerator();
                while (enumerator1.MoveNext())
                {
                    ClusterTest current = enumerator1.Current;
            ...
            switch (this.m_file.PathType)
            {
                ...
                case GH_ClusterFileType.Ghx:
                    ...
                    IEnumerator<ClusterTest> enumerator2;
                    try
                    {
                ...
                case GH_ClusterFileType.GhCluster:
                    ...
                    IEnumerator<ClusterTest> enumerator3;
                    try
                    {
        ...
        /// <summary>Gets the complete family of clusters of which this cluster is a part.</summary>
        /// <returns>A list of associated clusters (including this one)</returns>
        private IEnumerable<ClusterTest> ClusterFamily()
        {
            List<ClusterTest> ghClusterList = new List<ClusterTest>();
            GH_Document ghDocument = this.OnPingDocument();
            ...
            return (IEnumerable<ClusterTest>)ghClusterList;
        }
        ...
        {
            return this.m_password == null || (this.m_password.Length == 0 || ClusterTest.DoKeysMatch(this.m_password, ClusterTest.CreateKey(password)));
        }
        ...
        public bool AssignNewPassword(string oldPassword, string newPassword)
        {
            ...
            {
                this.m_password = ClusterTest.CreateKey(newPassword);
                flag = true;
            }
            return flag;
        }
        ...
        protected override void AppendAdditionalComponentMenuItems(ToolStripDropDown menu)
        {
            ...
            if (this.PreviewContent)
            {
                ...
                toolStripMenuItem2.Image = (Image)Res_GUI.LayoutVisible_20x20;
            }
            else
            {
                toolStripMenuItem2.ImageScaling = ToolStripItemImageScaling.SizeToFit;
                toolStripMenuItem2.Image = (Image)Res_GUI.LayoutInvisible_20x20;
            }
            ...
            int num2 = this.ClusterFamily().Count<ClusterTest>() - 1;
            ...
            GH_DocumentObject.Menu_AppendItem((ToolStrip)menu, "Export…", new EventHandler(this.MenuExportClicked), (Image)Res_MainMenu.SaveAs_20x20, this.m_internalDocument != null, false).ToolTipText = "Create a new *.ghcluster file based on this cluster.";
            GH_DocumentObject.Menu_AppendItem((ToolStrip)menu, "Export && Reference…", new EventHandler(this.MenuExportAndLinkClicked), (Image)Res_MainMenu.Save_20x20, this.m_internalDocument != null, false).ToolTipText = "Create a new *.ghcluster file based on this cluster" + Environment.NewLine + "and immediately link to that file.";
        ...
        private void MenuSetPasswordClicked(object sender, EventArgs e)
        {
            ...
            if (clusterPasswordWindow.ShowDialog((IWin32Window)Instances.DocumentEditor) != DialogResult.OK)
                return;
            byte[] key1 = ClusterTest.CreateKey(clusterPasswordWindow.OldPassword.Password);
            byte[] key2 = ClusterTest.CreateKey(clusterPasswordWindow.NewPassword.Password);
            if (!GH_Cluster.DoKeysMatch(this.m_password, key1))
                return;
            GH_UndoRecord record = new GH_UndoRecord("Cluster Password");
            Guid guid = Guid.NewGuid();
            IEnumerator<ClusterTest> enumerator;
            try
            {
                enumerator = this.ClusterFamily().GetEnumerator();
                while (enumerator.MoveNext())
                {
                    ClusterTest current = enumerator.Current;
        ...
        {
            this.RecordUndoEvent("Preview Content", (IGH_UndoAction)new ClusterTest.GH_ClusterPreviewDocumentAction(this));
        ...
        private void MenuDisassociatedClicked(object sender, EventArgs e)
        {
            if (this.m_internalDocument == null)
                return;
            this.RecordUndoEvent("Disassociate Cluster", (IGH_UndoAction)new ClusterTest.GH_ClusterDocumentIdAction(this));
        ...
        private void MenuExportAndLinkClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        ClusterTest current = enumerator.Current;
                        record.AddAction((IGH_UndoAction)new ClusterTest.GH_ClusterReferenceAction(current));
                        record.AddAction((IGH_UndoAction)new ClusterTest.GH_ClusterDocumentIdAction(current));
                        current.DocumentId = guid;
            ...
            switch (this.m_file.Synchronization)
            {
                ...
                case GH_Synchronisation.OutOfDate:
                    ...
                    try
                    {
                        ...
                        while (enumerator2.MoveNext())
                        {
                            ClusterTest current = enumerator2.Current;
        ...
        public void DisplayClusterPropertiesEditor()
        {
            ...
            try
            {
                ...
                {
                    ClusterTest current = enumerator.Current;
                    record.AddAction((IGH_UndoAction)new ClusterTest.GH_ClusterPropertiesUndoAction(current));
                    record.AddAction((IGH_UndoAction)new ClusterTest.GH_ClusterDocumentIdAction(current));
                    ...
                    if (this.m_internalDocument != null)
                    {
                        try
                        {
                            ...
                            while (enumerator.MoveNext())
                            {
                                IGH_PreviewObject current = enumerator.Current as IGH_PreviewObject;
                                if (current != null && !current.Hidden)
                                    ((BoundingBox) ref clippingBox).Union(current.ClippingBox);
                            }
                        }
                        finally
                        {
        ...
        public bool ReadGhClusterFile(string path)
        {
            ...
            try
            {
                ...
                ClusterTest ghCluster = new ClusterTest();
                ...
                else
                {
                    ...
                    ghCluster.m_mapping = (ClusterTest.HookParamMap)null;
        ...
        public class GH_ClusterPropertiesUndoAction : GH_ObjectUndoAction
        {
              ...
              : base(cluster.InstanceGuid)
            {
                ...
                if (cluster.m_icon_override == null)
                    return;
                this.m_icon = (Bitmap)cluster.m_icon_override.Clone();
            }
            ...
            protected override void Object_Undo(GH_Document doc, IGH_DocumentObject obj)
            {
                ...
                if (ghCluster.m_icon_override != null)
                    bitmap = (Bitmap)ghCluster.m_icon_override.Clone();
        ...
        {
            get { return new Guid("d4704e2b-e49d-4648-9f76-87abc75742e6"); }
        }
    }
}