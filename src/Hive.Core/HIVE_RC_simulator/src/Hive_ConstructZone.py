# This component creates a zone made of elements: the user is allowed 
...
"""
construct a customized zone using elements as inputs.
...
ghenv.Component.Name = "Hive_ConstructZone"
ghenv.Component.NickName = 'ConstructZone'
...
def zone_object_string(Zone,unique_inputs,thermal_attributes):
    ...
    if Zone.zone.elements is not None:
        external_envelope_area =  sum([e.area for e in Zone.zone.elements])
        wall_area = sum([e.area for e in Zone.zone.elements if e.opaque])
        u_walls = sum([e.area * e.u_value for e in Zone.zone.elements if e.opaque])/wall_area
        window_area =  sum([e.area for e in Zone.zone.elements if not e.opaque])
        if window_area >0:
            u_windows = sum([e.area * e.u_value for e in Zone.zone.elements if not e.opaque])/window_area
        else:
            u_windows = None
...
if elements ==[] or (volume or floor_area) is None:
    raise_error('Data missing: Elements, Zone volume or floor area are not specified.')
...
if any([type(e).__name__!='Element' for e in elements]):
    ...
    Zone1_string, Zone2_string = zone_object_string(Zone,unique_inputs,thermal_attributes)
    print 'The following is a Python-friendly definition of the Zone, compatible with the RC_BuildingSimulator Github repository:'
    print Zone1_string